...
const { slashRel, tablePath, ensureZkz, isLibRel, formatLocalNow } = require('./paths');
...
function saveTable(repoRoot, files, extra) {
  ...
  fs.writeFileSync(tablePath(repoRoot), JSON.stringify(obj, null, 2), 'utf8');
  return obj;
}
...
function setKind(repoRoot, rel, kind) {
  const files = loadTable(repoRoot);
  files[slashRel(rel)] = kind;
  saveTable(repoRoot, files);
  return files;
}
