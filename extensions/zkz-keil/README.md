...
| H750 | 存在 `H750/Projects/MDK-ARM/H750_N.uvprojx`（可用 `zkz-keil.flavor` 覆盖） | 核 QSPI hex；J-Link + W25Q32 FLM |

UV4 / J-Link / FLM / 设备名 / 速度 / QSPI 窗口写在设置 `zkz-keil.*`（空则环境变量，再试常见安装路径）。
...
打开真源工程时，状态栏有 `zkz-keil`，点击弹出上述命令。

`zkz-keil.resolveLayout` 给其它扩展取工程/产物路径，不出现在命令面板。
