...
async function publishDebugArtifacts(root, layout, emit) {
  ...
  const nativeOn = fs.existsSync(path.join(root, '.zkz', 'keil-native'));
  ...
  emit('  gdb map true source -> ' + root + '/.zkz');
}

async function resolveWorkspace() {
  const r = await invokeOptional('zkz-native.resolveRoot', null);
  if (r.found && r.result && r.result.root) return { root: r.result.root };
  return resolvePair();
}
